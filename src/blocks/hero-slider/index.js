...
registerBlockType( metadata.name, {
	edit( { attributes, setAttributes } ) {
		...
		// Derive the current height unit from the saved values first,
		// then fall back to heightMode. This keeps the UI consistent
						...
						<ResponsiveUnitControl
							...
							onChange={ ( next ) =>
								setAttributes( {
									...
									headingFontSizeUnit:
										next.unit || 'px',
									// Keep legacy single value in sync (desktop).
									headingFontSize:
										next.values.desktop !== ''
											? parseFloat(
													next.values.desktop
