...
/**
 * Parse a stored px value for RangeControl. Zero is valid; do not use `|| fallback`.
 *
 * @param {string|number|undefined} raw Stored attribute (e.g. "8px", 0, "0px").
 * @param {number}                  fallback When missing or non-numeric.
 * @return {number}
 */
...
registerBlockType( metadata.name, {
	edit: ( { attributes, setAttributes } ) => {
		...
		// Fetch all classic navigation menus (Appearance → Menus).
					...
					>
						...
						<ButtonGroup>
							...
							<Button
								...
								onClick={ () =>
									setAttributes( {
										submenuTextAlignment: 'center',
									} )
								}
								aria-label={ __(
									'Align dropdown text center',
									'croco-blocks'
								) }
							/>
							<Button
								...
								onClick={ () =>
									setAttributes( {
										submenuTextAlignment: 'right',
									} )
								}
								aria-label={ __(
									'Align dropdown text right',
									'croco-blocks'
								) }
							/>
						</ButtonGroup>
